...
def create_policies_in_oss(vector_store_name, aoss_client, bedrock_kb_execution_role_arn):
    ...
    try:
        ...
        print(f"Encryption policy {encryption_policy_name} already exists. Retrieving existing policy.")
        policies = aoss_client.list_security_policies(type='encryption')
        policy_list = policies.get('securityPolicySummaries', [])
        # Try to find an exact match
        encryption_policy = next((p for p in policy_list if p.get('name') == encryption_policy_name), None)
        # Fallback: use the most recent one if an exact match isn't found
        if encryption_policy is None and policy_list:
            encryption_policy = max(policy_list, key=lambda p: p.get('createdDate', 0))
        if encryption_policy is None:
            raise ValueError("Encryption policy exists but could not be retrieved.")
    ...
    try:
        ...
        print(f"Network policy {network_policy_name} already exists. Retrieving existing policy.")
        policies = aoss_client.list_security_policies(type='network')
        policy_list = policies.get('securityPolicySummaries', [])
        network_policy = next((p for p in policy_list if p.get('name') == network_policy_name), None)
        if network_policy is None and policy_list:
            network_policy = max(policy_list, key=lambda p: p.get('createdDate', 0))
        if network_policy is None:
            raise ValueError("Network policy exists but could not be retrieved.")
    ...
    try:
        ...
        print(f"Access policy {access_policy_name} already exists. Retrieving existing policy.")
        policies = aoss_client.list_access_policies(type='data')
        print("Access policies response:", json.dumps(policies, indent=2))
        
        # Try both possible keys
        policy_list = policies.get('accessPolicyDetails') or policies.get('accessPolicySummaries') or []
        
        access_policy = next((p for p in policy_list if p.get('name') == access_policy_name), None)
        if access_policy is None and policy_list:
            access_policy = max(policy_list, key=lambda p: p.get('createdDate', 0))
        if access_policy is None:
            raise ValueError("Access policy exists but could not be retrieved.")
...
def orig_create_policies_in_oss(vector_store_name, aoss_client, bedrock_kb_execution_role_arn):
    encryption_policy = aoss_client.create_security_policy(
        name=encryption_policy_name,
        policy=json.dumps(
            {
